...
マッチするシンボルだけを逆アセンブルできます。@<fn>{objdump}
...
@<code>{defer}文をみつけると@<code>{deferBits}変数の対応するビットを立て、
...
静的単一代入形式とは、コンパイラの最適化で用いられる形式です。
変数への代入を一度だけになるように制限した形式になります。
Goのコンパイラでも静的単一代入形式にすることで
最適化がしやすくなるため、1.7から導入されました。@<fn>{ssa}

//footnote[ssa][@<href>{https://golang.org/doc/go1.7#compiler}]

Goのコンパイラでは、コードを静的単一代入形式に一度変換します。
そして、そこに順に最適化処理をかけていき、
最終的なオブジェクトコード（バイナリ）を生成します。
...
@<code>{runtime.deferreturn}関数などが呼ばれているか分かります。
