...
主にGit拡張の@<tt>{git-codereview}や、コードレビューツールであるGerrit@<fn>{gerrit}の使い方について解説します。
...
内容としては、次のようなことを含むのが一般的です。
  ...
  * このCLが導入される前の問題は何なのか。
  * このCLは何をするのか、または問題をどう解決するのか。
  * このCLが導入されたあと、どういう状態になるのか。
  * 関連するIssue。

文脈をまったく知らない人がこのCLを見たときに、情報を把握するために必要です。
多少冗長でも明確な説明が好まれます。

関連するIssueは@<tt>{Updates}または@<tt>{Fixes}と書き、その後にIssue番号を@<tt>{golang/go#32963}のように書きます。
