...
Raftの論文においてはリーダーの選出方法が先に説明がありますが、本章では理解しやすさのためにログ複製の管理をどのように行っているかを先に説明します。Raftは最初に1人のリーダーを選出し、複製されたログの管理に関する完全な責任をリーダーに与えることによってコンセンサスを実現します。クライアントからのリクエストは必ずリーダーが受け取ります(@<img>{logrep1})。もしリクエストがフォロワーに届いてもリクエストをリーダーに渡します。
