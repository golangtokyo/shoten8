...
これでクラスターの動的な設定変更機能が完成しました。さらなる応用として、etcdはv3.4からRaftに「リーダー、フォロワー、候補者」に次ぐ4つ目の状態である@<b>{learner（学習者）}@<fn>{learner}を実装しています。このアイデア自体はコンセンサスアルゴリズムの実践を考察した論文@<fn>{raft-plactice}の§4.2.1で@<b>{non-vorting member（非投票者）}として導入されています。これはノードの新規追加時にリーダーのログに追いつくまで投票権のないメンバーとしてクラスターに参加させるというアイデアです。この方法はログ複製でリーダーが過負荷になるのを防ぎ、クラスター再構成の安全性を高めます。
