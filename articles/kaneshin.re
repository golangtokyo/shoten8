...
ファイル名は、ファイル名の昇順でわかりやすいものを定義しているとの、出力と呼び出された関数オブジェクトの名前を取得するために次の変数を定義しています。
...
初期化を完全に管理下に置きたい場合はinit.goなどにまとめるのがよいですが、変数や@<code>{init}関数、そして依存パッケージも上から順に再帰的に処理されます。このように順序関係を意識する必要のある初期化処理や、ほかの初期化処理に依存する初期化処理が必要となる設計はそもそも見直しが必要です。すべてを実行時に初期化処理できればよいですが、ある程度妥協することも設計の上では大事になります。複雑性を解決するためのGoが、逆に複雑性を上げてしまうことはよいとはいえません。
