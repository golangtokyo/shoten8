...
Goの作法ではパッケージの初期化と、パッケージ間の依存関係を意識することになります。実行時にパッケージの変数を初期化することによって変数の確保や処理時間を効率化することができます。
パッケージの初期化が多いと@<code>{main}関数が実行されるまで待ち時間やメモリのスコープが関数ではなくパッケージスコープとなることによって変数を参照のコストが高くなることがあります。
このようなデメリットを理解した上で初期化をすることは効率性を向上させるために重要です。
...
それぞれのファイルにある変数の初期化が処理され、その後に@<code>{init}関数が処理されています。@<code>{main}関数のあるファイルが最後に初期化処理されるわけではありませんし、main.goやinit.goというファイル名を作ってもファイル名による優先処理はありません。
...
このようにハンドリングする必要のあるエラーは関数内で生成して返却するのではなく、パッケージスコープとして宣言したものを返却することでパッケージの外からハンドリングすることができます。
