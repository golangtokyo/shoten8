...
Goでは@<code>{main}関数が実行されるまでにパッケージの変数や@<code>{init}関数を初期化して実行処理を効率的に行っています。Goにはファイル単位でのスコープは存在せず、パッケージのスコープで変数や関数が参照される仕様のため、コンパイラの仕様に依存して実行時の初期化が行われています。基本的には、このコンパイラの呼び出し順序の仕様に依存しない初期化や依存関係を考慮した実装をすべきです。良い設計をするために知っておいて損はない処理だと考えています。
...
Goの作法ではパッケージの初期化と、パッケージ間の依存関係を意識することになります。実行時にパッケージの変数を初期化することによって変数の確保や処理時間を効率化することが可能になりますが、パッケージの初期化が多いと@<code>{main}関数が実行されるまで待ち時間が長くなってしまうことや初期化の順序、そしてメモリのスコープが関数ではなくパッケージスコープとなることによって変数を参照するオーバーヘッドに時間が掛かってしまうことがあります。このようなデメリットを理解した上で初期化をすることは効率性を向上させるために重要です。

=== 初期化の順序

@<code>{init}関数の実行順序は上から順に実行されますが、パッケージに複数のファイルやパッケージ変数が定義されていたり、依存パッケージが存在するときの初期化順序について知らない人も多いのではないでしょうか。パッケージの初期化はパッケージ変数の初期化の後に@<code>{init}関数の実行という順序で行われます。
...
ファイル名は、ファイル名の昇順でわかりやすいものを定義しているとの、出力と呼び出された関数オブジェクトの名前を取得するために下記の変数を定義しています。
...
結果は、それぞれのファイルにある変数の初期化がはじめに処理され、その後に@<code>{init}関数が処理されています。Goはパッケージスコープなので、@<code>{main}関数のあるファイルが最後に初期化処理されるわけではありませんし、main.goやinit.goというファイル名を作ってもファイル名による優先処理はありません。
...
@<code>{import _ "github.com/kaneshin/foo/bar/dep"}のようにブランクインポートさせると下記のように先に依存パッケージの初期化処理が行われて、依存元のパッケージの初期化が処理されます。
...
初期化を完全に管理下に置きたい場合はinit.goなどにまとめるのが良いですが、変数や@<code>{init}関数、そして依存パッケージも上から順に再帰的に処理されます。このように順序関係を意識する必要のある初期化処理や、他の初期化処理に依存する初期化処理が必要となる設計はそもそも見直しが必要です。すべてを実行時に初期化処理することができれば良いかもしれませんが、ある程度妥協することも設計の上では大事になります。複雑性を解決するためにGoが開発されたのに、逆に複雑にしては元も子もありません。

== パッケージスコープの変数宣言と初期化

パッケージスコープの変数を宣言させることはよいことなのか？Goは並行処理される言語のためパッケージスコープの変数へは競合状態を気にする必要があります。そのため、パッケージスコープな変数は@<code>{goroutine}間での処理を意識しながら開発と実行をしなくてはならなくなるため、密結合の温床であり依存性の高いコードとなるのであまり推奨されません。
...
メモリキャッシュをするときもパッケージスコープに@<code>{map}や@<code>{map}をメンバ変数とした@<code>{struct}を変数としてキャッシュにすることもあります。並行処理で使われることが前提になるため競合状態のハンドリングを忘れずに実装する必要があります。@<code>{sync.Map}などを参考にしてみてください。
...
Goにおける初期化について、パッケージスコープの変数と@<code>{init}関数、初期化の順序について紹介しました。これらを気にすることなくコーディングすることは可能ですが、細かいところまでケアしてコードを書くことによってメモリや処理の最適化がされますし、開発における設計の複雑性除去の判断にもつながると筆者は考えています。
