...
テンプレート機能とは、出力形式をあらかじめ定義しておき、出力する時に定義された形式に沿って出力を組み立てる機能です。

テンプレート機能はwebサービスやコード生成など様々な場面で利用されています。
@<code>{go list}コマンドでも結果を出力する際にテンプレート機能を利用することで、利用者が出力結果を自由に変更することができます。

Goのテンプレート機能はブロックの定義や、自作関数の適用など複雑なテンプレートも作成することができます。
テンプレートで複雑な処理を行うと実行した時にどのような形式になるか予測しづらくなります。
テンプレートによる出力をを実行する前に、ミスが無いかなどをチェックする必要があります。
...
この構文が@<code>{true}になる場合にその変数を利用していなければ、意味がない条件になります。
意味のない条件を防ぐために、条件をチェックし実行する前にエラーを出します。
...
Badの場合ではwith句で@<code>{Name}変数をチェックしたにも関わらず、@<code>{Age}変数を出力しています。
Badのようなテンプレートの場合警告が出るようにチェックします。

テンプレートの静的解析は、Goコードの静的解析のように実行する前に別のツールとして実行するのは大変です。
...
2. with句が@<code>{true}の場合の部分で使われている変数を探し出す。
...
@<code>{WithNode}は更に@<code>{BranchNode}@<fn>{branchnode_document_link}という型を埋め込んでいます。
...
@<list>{example_with_statements}の4〜6行目は@<code>{Decl}フィールドで確認することができます。
1〜3行目は@<code>{Cmds}フィールドから確認することができます。
@<code>{*parse.CommandNode}型のスライスなので、ループでチェックしていきます。
@<code>{*parse.CommandNode}型の引数を表す@<code>{Args}フィールドでは様々な種類のノードが入る可能性があります。
しかし実際にチェックする必要があるのは、@<code>{*parse.FieldNode}型か@<code>{*parse.DotNode}型のみです。
@<code>{*parse.DotNode}型はバリエーションが無いため、シンプルです。
...
@<code>{*parse.FieldNode}や@<code>{*parse.VariableNode}、@<code>{*parse.ChainNode}は要素名がスライスになっているので、ドットでつなげて一つにします。
...
=== 結果を返す
with句でチェックした変数が@<coode>{nil}でない場合にその変数が正しく使われているかチェックすることはできました。
最後にその結果をわかりやすく返す必要があります。

withcheckでは@<coode>{error}型で返すようにしました。
...
静的解析するための環境もGo本体ほどでは有りませんが、揃っています。
