...
埋込み型による疑似継承のアプローチもありますが、これは多態性や共変性、反変性を満たさないのでオブジェクト指向で期待される@<kw>{実装継承}（@<i>{Implemebtation inheritance}）ではありません。
...
このことは代表的なオブジェクト指向言語である@<kw>{Java}の名著、「@<kw>{Effective Java}」@<fn>{amzn_java}の「項目15 継承よりコンポジションを選ぶ」でも言及されています。
