...
このような継承関係（ポリモフィズム）を表現するとき、@<tt>{Go}はインターフェースを使うでしょう。
@<tt>{Go}は@<list>{person}のような実像クラス（あるいは抽象クラス）を親とするようなサブクラシングによる継承の仕組みを言語仕様としてサポートしていません。
...
//}
以上の例以外にも、@<kw>{リスコフの置換原則}などの一部のSOLIDの原則をそのまま@<tt>{Go}に適用することはできません。
...
ドメインやパッケージの単位の単一責任になってくると解釈が一概に言えるものではなくなります。
...
@<tt>{SRP}と@<tt>{Go}の言語思想である単純さが求める姿が近いと言えるのではないでしょうか。
...
　変更理由が1つしか存在しないということは、その型の責務（責任）がひとつであることを意味します。
...
@<tt>{SRP}は1番単純な原則です。
しかし、実際にコードや問題に向き合ったときに「ある役割のかたまり」や「単一責任」を定義するのは難しいです。
これはドメイン駆動開発やマイクロサービスにおける「境界」探しの難しさにも通じています。
同じ処理（ドメイン）でもフレームワーク（組織構造）などの環境やその時の状況によって「単一責任」の定義が変わるかもしれません。
@<tt>{SRP}を考えることは設計の本質です。正解はないからこそ、経験や他の原則を駆使して「単一責任」の定義に立ち向かう必要があります。
@<tt>{UNIXという考えかた}@<fn>{unix}でもある「ひとつのことをうまくやれ」も@<tt>{SRP}と同様の思想です。
「ひとつのこと」「ドメイン」など様々な文脈の「単一責任」を学ぶことが@<tt>{SRP}を実践していくために必要です。

#@# GoとSOLID、的には、標準コードよんでがんばろ！みたいな結論でもいいのかもしれない
...
そのため、クラス間の継承（サブクラシング）について言及している@<tt>{LSP}を@<tt>{Go}に適用はできません。
しかし、@<tt>{LSP}はもとに@<kw>{振る舞い}と@<kw>{契約による設計}という哲学があります。
この哲学を@<tt>{Go}のコードに取り入れることは可能であり、よりよいコードを書くために役立つでしょう。

=== @<tt>{サブクラシング}による継承と@<tt>{振る舞い}
　（@<tt>{Go}にはありませんが、）@<tt>{サブクラシング}で継承関係をもたせたとき、子クラスは親クラスの機能や構造を子クラス自身に定義されているかのように振る舞うことができます。
場合によっては@<tt>{オーバーライド}によって振る舞いを変更することも可能です。
子クラスにとってメリットしかないように聞こえる継承ですが、本来@<tt>{子クラスは親クラスに期待される振る舞いを守る}義務があります。
...
@<tt>{Go}にはサブクラシングによる継承はありませんが、インターフェースを利用した継承があります。
