...
== @<kw>{SOLIDの原則}（the SOLID principles）とは
...
==={no_subclassing} @<kw>{Go}はサブクラシング（@<kw>{subclassing}）に対応していない
...
== @<kw>{単一責任の原則}（@<kw>{SRP}, @<kw>{Single responsibility principle}）
//quote{
...
== @<kw>{オープン・クローズドの原則}（@<kw>{OCP}, @<kw>{Open–closed principle}）
//quote{
...
== @<kw>{リスコフの置換原則}（@<kw>{LSP}, @<kw>{Liskov substitution principle}）
//quote{
...
=== @<kw>{Go}の標準インターフェースから読み解く@<kw>{契約による設計}
...
=={isp} インタフェース分離の原則（@<kw>{ISP}, @<kw>{Interface segregation principle}）
//quote{
...
== @<kw>{依存関係逆転の原則}（@<kw>{DIP}, @<kw>{Dependency inversion principle}）
//quote{
...
=== @<kw>{database/sql/driver}パッケージと@<kw>{DIP}
