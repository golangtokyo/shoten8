...
==== OverlayFS
@<hd>{ファイルシステムの隔離}では@<code>{pivot_root}や@<code>{mount}を使い、コンテナに対して新たなファイルシステムを割り当て、コンテナからホストのファイルシステムを見えないようにしました。しかしコンテナのファイルシステムはあくまでもホストのルート下に存在しており、コンテナ内でファイルやディレクトリの作成・上書き・削除などを行うと、それがホストにも影響します。
