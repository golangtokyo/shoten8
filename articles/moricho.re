...
ブロックデバイスのデータを人が直接扱うのは大変ですが、ファイルシステムがそれらを@<b>{ファイル}や@<b>{ディレクトリ}という形で扱うことを可能にしています。
またブロックデバイスのデータを、システム上のディレクトリツリーに対応させることを@<b>{マウント}といいます。
...
まず「new_rootとput_oldは現在のrootと同じファイルシステムにあってはならない」を満たすために、new_rootを新たなマウントポイントとしてnew_root自身でバインドマウントしています。
マウントは通常、ディレクトリツリーをブロックデバイスの領域に紐付けるために行われます。
それに対し「バインドマウント」は、ディレクトリをディレクトリにマウントします。
今回はnew_rootをnew_rootでマウントすることで、new_root以下の階層の内容はそのままに、new_rootを新たなマウントポイントとしたファイルシステムとして認識させています。

そしてこの後にpivot_rootを行うことで、新たにnew_rootがファイルシステムのルートになり、その上位階層（ホストのディレクトリ）は見ることができなくなります。
またpivot_rootでは元のファイルシステムがput_oldをマウントポイントとしてマウントされるため、コンテナにホストの情報が残ったままになってしまいます。これを回避するために、続いてput_oldをアンマウントしてから削除しています。
